...
public function element_form_register_my_custom_menu_page(){
    add_menu_page( 
        __( 'Form Submissions', 'submissions' ),
        'Form Submissions',
        'manage_options',
        'submissions',
		 array($this,'submissions'),
		'dashicons-editor-table',
        6
	); 

    add_submenu_page('options.php', 'View Submission', 'View Submission',
			'manage_options', 'view_submission', array( $this, 'view_submission' ) );
}
 ...
 */
 public function submissions(){
	require_once plugin_dir_path( dirname( __FILE__ ) ) . 'admin/partials/submissions.php';
}
 
public function view_submission() {
	require_once plugin_dir_path( dirname( __FILE__ ) ) . 'admin/partials/view_submission.php';
}
...
public function  db_elementor_form_new_record( $record, $form_class ) {
	...
	if ( $fields = $record->get_formatted_data() ) {
		...
		$datas = array(
			...
			'submitedBy' => sanitize_text_field($current_user),
			'formData' => sanitize_text_field($serialized_data)
		   );
...
function void_db_elemetor_call_after_for_submit( $contact_data ){
	...
	if ($submission){
		...
		$datas = array(
			'submitedOn' => sanitize_text_field($this_page->post_title),
			'formID' => sanitize_text_field($contact_data->id),
			'postID' => sanitize_text_field($this_page->ID),
			'email' => $email,
			'message' => $message,
			'submitedBy' => sanitize_text_field($current_user),
			'formData' => sanitize_text_field($serialized_data)
 ...
 public function void_db_elemetor_call_after_for_submit_gravity_form($entry, $form){
   ...
   foreach ( $form['fields'] as $field ) { 
	...
	$datas = array(
		'submitedOn' => sanitize_text_field($this_page->post_title),
		'formID' => sanitize_text_field($form['title']),
		'postID' => sanitize_text_field($this_page->ID),
		'email' => sanitize_email($email),
 ...
 // Ninja form
	...
	foreach ( $data['fields'] as $field ) {
		...
		$datas = array(
			'submitedOn' => sanitize_text_field($data['settings']['title']),
			'formID' => sanitize_text_field($data['id']),
			'postID' => '',
			'email' => sanitize_email($email),
			'message' => sanitize_text_field($message),
			'submitedBy' => sanitize_text_field($current_user),
	...
	public static  function getAllDBFromData (){
		...
		if($wpdb->get_var("SHOW TABLES LIKE '$table'") == $table) {
			$datas = $wpdb->get_results("SELECT * FROM $table WHERE status = '$status' ORDER BY id DESC");
	...
	public static  function deleteViewSubData($ID){
		...
		$table = $wpdb->prefix.'db_element_form';
		$datas = $wpdb->query("DELETE FROM $table WHERE id = $ID");
		echo '<script> window.location="'. admin_url('admin.php?page=submissions') .'" </script>';
	 }
