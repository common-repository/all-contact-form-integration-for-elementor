 ...
 *
 * @wordpress-plugin
 * Plugin Name:       EleForms - All In One Form Integration with DB for Elementor
 * Plugin URI:        https://wordpress.org/plugins/all-contact-form-integration-for-elementor/
 * Description:       Integrate, Style and Store Form Submissions of your Contact Form 7, Ninja Forms, Gravity Forms, Elementor Form. Get SMS notification on form submissions. Never miss a lead!
 * Version:           2.9.9.9
 * Author:            TheInnovs
 * Author URI:        https://theinnovs.com
 * License:           GPL-2.0+
 * License URI:       http://www.gnu.org/licenses/gpl-2.0.txt
 * Text Domain:       eleforms
 ...
 */
define( 'DB_Elementor_Form', '2.9.9.9' );
...
function void_db_elementor_activate_notice() {
	...
	if ( !in_array( 'elementor/elementor.php', $active_plugins ) ) {
		$message = "You have to install Elementor plugin to run EleForms";
    ?>
...
function cssefe_activate_notice() {
    ...
    <div class="updated notice notice-my-class is-dismissible cssfe_notice">
        <p><?php _e( '<strong></strong> Try <strong><a href=" '. $plugin_url . '">Elements CSS </a></strong> Addons for Elementor and get <strong> additional widgets as well as Elementor Widgets</strong> more EXTENDED. (1K+ active users) <br>Now creating Sticky Header,Hover Effects, Gradients, Custom link in Image Carousel (each image), Sticky Footer & so on is a matter of second!', 'eleforms' ); ?></p>
    </div>
    <?php
  }
...
function elemento_forms_settings_link( $links_array, $plugin_file_name ){
  ...
  if( strpos( $plugin_file_name, basename(__FILE__) ) ) {
    ...
    array_unshift( $links_array, '<a href="https://theinnovs.com/hire-elementor-experts/" style="color: #389e38;font-weight: bold;" target="_blank">' . __( 'Hire Elementor Experts', 'eleforms' ) . '</a>' );

    array_unshift( $links_array, '<a href="https://theinnovs.com/contact/" style="color: black;font-weight: bold;" target="_blank">' . __( 'Plugin Support', 'eleforms' ) . '</a>' );

     array_unshift( $links_array, '<a href="https://theinnovs.com/downloads/elementorsms/?discount=elesms60" style="color: red;font-weight: bold;" target="_blank">' . __( 'Get Elementor SMS Addon', 'eleforms' ) . '</a>' );
